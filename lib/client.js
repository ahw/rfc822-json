 ...
 * @param {Number} uid Message identifier
 */
IMAPClient.prototype.createMessageStream = function(uid, options){
    var stream = new Stream();

    uid = Number(uid) || 0;
    // Flag to indicate whether to parse into JSON or send as raw stream.
    var returnAsJson = (options && options.json) ? options.json : false;
    ...
    if (returnAsJson) {
        // If we're rendering as JSON, then pipe the raw message stream
        // through the MessageParser, which itself is a readable stream of
        // JSON data.
        return stream.pipe(new MessageParser());
    } else {
        return stream;
    }
};
